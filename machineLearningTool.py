# -*- coding: utf-8 -*-
"""
Created on Tue Mar  7 11:18:38 2021
    ...
    def baseFilter(self, columnNames, aboveVal, belowVal, naDecision):
        ...
        for (columnName, columnData) in self.filtered.items():
            ...
            if (isinstance(columnData.values[0], numbers.Number)):
                ...
                print(columnName)
                print(
                    "This column is not a int OR it is not a numpy int/float, make sure you apply any preprocessing to it manually")
...
class model:
    ...
    def searchCV(self, n_iter, cv, searchDsetSize):
        ...
        if (self.param_dist != 'NA'):
            ...
            self.search.fit(self.data.X_train[0:int(len(self.data.X_train) / searchDsetSize), :],
                            self.data.y_train[0:int(len(self.data.X_train) / searchDsetSize)])
    ...
    def train(self, trainingDsetSize, iterators):
        ...
        if (self.modelType.upper() == 'XGB'):
            ...
            self.model = xgb.XGBRegressor(**self.search.best_params_, n_estimators=iterators,
                                          objective='reg:squarederror', tree_method='gpu_hist', random_state=42,
                                          verbose=True)

            # Train the model on the full training data
            self.model.fit(self.data.X_train[0:int(len(self.data.X_train) / trainingDsetSize)],
                           self.data.y_train[0:int(len(self.data.y_train) / trainingDsetSize)], verbose=True)
        elif (self.modelType.upper() == 'SVM'):
            self.model = SVR(kernel='rbf', C=self.search.best_params_['C'], gamma=self.search.best_params_['gamma'])
            self.model.fit(self.data.X_train[0:int(len(self.data.X_train) / trainingDsetSize)],
                           self.data.y_train[0:int(len(self.data.y_train) / trainingDsetSize)], verbose=True, n_jobs=-1)
            ...
            if (len(self.data.X_valid != 0)):
                ...
                self.history = self.model.fit(self.data.X_train[0:int(len(self.data.X_train) / trainingDsetSize)],
                                              self.data.y_train[0:int(len(self.data.y_train) / trainingDsetSize)],
                                              epochs=iterators, callbacks=[self.es],
        ...
        # Models to make: BIRCH, K_Means, Spectral, Mixture of Gaussians, Maybe: Optics
            ...
            self.initgroupPreds = self.model.predict(
                np.ascontiguousarray(self.data.X_train[0:int(len(self.data.X_train) / trainingDsetSize)]))
        ...
        for i in range(self.num_layers):
            ...
            while j < self.num_models:
                ...
                if (isinstance(self.modelList[i][j], str)):
                    ...
                    self.dataLayerTrain[i].Append(self.modelList[i][j].model.predict(self.modelList[i][0].data.X_train),
                                                  (self.modelList[i][j].modelType + self.TmpName))
                    self.dataLayerTest[i].Append(self.modelList[i][j].model.predict(self.modelList[i][0].data.X_test),
                                                 (self.modelList[i][j].modelType + self.TmpName))
                    self.dataLayerValid[i].Append(self.modelList[i][j].model.predict(self.modelList[i][0].data.X_valid),
                                                  (self.modelList[i][j].modelType + self.TmpName))
                elif (j == 0):
                    print("j is 0")
                    ...
                    print("i is 0")
                    self.TmpName = str("Layer" + str(i) + "_Model" + str(j))
                    self.dataLayerTrain[i].Append(
                        self.modelList[i][j].model.predict(self.modelList[i][0].data.X_train[baseInput]),
                        (self.modelList[i][j].modelType + self.TmpName))
                    self.dataLayerTest[i].Append(
                        self.modelList[i][j].model.predict(self.modelList[i][0].data.X_test[baseInput]),
                        (self.modelList[i][j].modelType + self.TmpName))
                    self.dataLayerValid[i].Append(
                        self.modelList[i][j].model.predict(self.modelList[i][0].data.X_valid[baseInput]),
                        (self.modelList[i][j].modelType + self.TmpName))
                else:
                    print("i and j are not 0")
                    self.TmpName = str("Layer" + str(i) + "_Model" + str(j))
                    self.dataLayerTrain[i].Append(self.modelList[i][j].model.predict(self.dataLayerTrain[i - 1].df),
                                                  (self.modelList[i][j].modelType + self.TmpName))
                    self.dataLayerTest[i].Append(self.modelList[i][j].model.predict(self.dataLayerTest[i - 1].df),
                                                 (self.modelList[i][j].modelType + self.TmpName))
                    self.dataLayerValid[i].Append(self.modelList[i][j].model.predict(self.dataLayerValid[i - 1].df),
                                                  (self.modelList[i][j].modelType + self.TmpName))
    ...
    def startCluster(self):
        ...
        if (self.clusterSearch):
            for i in range(len(self.clusterName)):
                self.clustModel[i] = model(self.cluster[i], self.clusterName[i], self.cluster_params[i][0], self.data,
                                           True)
    ...
    def createData(self, prepTrain, data='na'):
        ...
        if prepTrain:
            if (self.clusterSearch):
                ...
                self.newData = sortGroups(self.data.X_test, self.predTMPTrain, self.predTMPTest,
                                          self.chosenClustModel.groups)
            else:
    ...
    def testSecond(self):
        ...
        if (self.clusterSearch):
            ...
            for i in range(len(self.models)):
                self.results.update(
                    {i: self.models[i].modPredict(self.newData[i].X_test, self.newData[i].y_test, True)})
