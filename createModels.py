...
def createData(goal, modelType, group):
    ...
    if modelType.upper() == 'SIMPLE':
        ...
        data.lagFuture(columns['y'], dataPack['laggedVars'], dataPack['notLaggedVars'], dataPack['numPastSteps'],
                       dataPack['numFutureSteps'])
        if not dataPack["scaleVal"] == 'na':
            print("SCALE DA DATA")
...
def createModel(goal, modelType, group):
    ...
    if structure.upper() == 'REGULAR':
        ...
        simpModel = aim.breakout(modelPack['clusterSearch'], modelPack['cluster'], modelPack['clusterName'],
                                 modelPack['cluster_params'], modelPack['run_params'], initData)
