...
def initDefaultSimulation(load, save=False):
    if not load:
        ...
        #Not necessary unless during testing
        ...
        if save:
            raise Exception("Save not yet developed, must train models on each run")
    elif load:
        raise Exception("Load not yet developed, must train models on each run")
...
def createDefaultSolar():
    ...
    dataPack.update({'numPastSteps': 2})
...
def createDefaultElectricityDemand():
    ...
    dataPack.update({'numFutureSteps': 30})
    tmpScaleVal = [0, 4000]
    dataPack.update({'scaleVal': tmpScaleVal})
...
# initDefaultSimulation(load=False, save=False)
