...
def livePredict(timeStamp, goal, modelType, group, numPast=0):
    try:
        ...
        X = X_train.iloc[[timeStamp - int(numPast*2)]]
        val = dataModels[goal][modelType][group]['model'].model.predict(X)
        return val
    except:
        X_test = dataModels[goal][modelType][group]['initData'].X_test
        X = X_test.iloc[[timeStamp - int(numPast*2) - len(dataModels[goal][modelType][group]['initData'].X_train)]]
        val = dataModels[goal][modelType][group]['model'].model.predict(X)
        return val
...
def defPredict(goal, modelType, group):
    ...
    results, predValues = dataModels[goal][modelType][group]["model"].modPredict(X, y, returnResults=True,
                                                                                 returnPred=True)
    print(results)
    print(predValues)
